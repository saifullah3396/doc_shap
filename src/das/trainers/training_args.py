...
class Optimizer:
    ...
    def create(self, model_param_groups, lr_scale=1):
        ...
        opt = None
        if self.name == OptimizerEnum.ADAM:
            opt = torch.optim.Adam(model_parameters, **base_params)
        elif self.name == OptimizerEnum.ADAM_W:
...
@dataclass
class RayTuneArguments:
    config: dict = field(
        default_factory=lambda: {
            "opt_params.default.default.lr": [1e-5, 5e-5],
            "batch_size": [32, 64, 128],
        }
    )
    cb_report: dict = field(
        default_factory=lambda: {"loss": "vl", "mean_accuracy": "vacc"}
    )

    def __post_init__(self):
        for key, value in self.config.items():
            if "opt_params" in key:
                self.config[key] = tune.loguniform(*value)
            elif key == "batch_size":
                self.config[key] = tune.choice(value)
...
class TrainingArguments:
    ...
    ray_tune: Optional[RayTuneArguments] = field(
        default=None,
        metadata={"help": "Ray tune parameters that define search space."},
    )
