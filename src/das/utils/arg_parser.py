...
from das.model_analyzer.analyzer_args import (
    AnalysisTaskArguments,
    AnalysisTaskArgumentsFactory,
    AnalyzerArguments,
)
    ...
    def parse_yaml_file(
        ...
        with open(file) as f:
            ...
            for dtype in self.dataclass_types:
                ...
                elif dtype == AnalyzerArguments:
                    for idx, model in enumerate(data[dtype.cls_name]["models"]):
                        model_type = ModelArgumentsFactory.create_model_arguments(
                            model["model_name"]
                        )
                        data[dtype.cls_name]["models"][idx] = from_dict(
                            data_class=model_type, data=model
                        )

                    for idx, analysis_task_arguments in enumerate(
                        data[dtype.cls_name]["analysis_tasks"]
                    ):
                        analysis_task = (
                            AnalysisTaskArgumentsFactory.create_child_arguments(
                                analysis_task_arguments["task_name"]
                            )
                        )
                        data[dtype.cls_name]["analysis_tasks"][idx] = from_dict(
                            data_class=analysis_task, data=analysis_task_arguments
                        )
